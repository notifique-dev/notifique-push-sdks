...
export function parsePushPayload(raw: unknown): PushIncomingPayload {
  ...
  const url = pickString(record, ["url"]) ?? pickString(data, ["url"]);
  ...
  return {
    ...
    logId,
    clickReportUrl: pickString(data, ["click_report_url", "clickReportUrl"]),
    deliveryReportUrl: pickString(data, ["delivery_report_url", "deliveryReportUrl"]),
    data,
  };
}
